...
resource "aws_instance" "opa_instance" {
    ...
    subnet_id = (var.subnet == "" ? element(split(",", local.subnet[var.vpc_name]), length(data.aws_instances.provisioned_instances.ids) % length(local.subnet[var.vpc_name])) : var.subnet)
...
data "aws_subnet" "selected" {
  id = var.subnet == "" ? element(split(",", local.subnet[var.vpc_name]), length(data.aws_instances.provisioned_instances.ids) % length(local.subnet[var.vpc_name])) : var.subnet
}
