...
resource "aws_instance" "opa_instance" {
    ...
    subnet_id = (var.subnet == "" ? element(split(",", local.subnet[var.vpc_name]), length(data.aws_instances.provisioned_instances.ids) % 3) : var.subnet)
...
data "aws_subnet" "selected" {
  id = var.subnet == "" ? element(split(",", local.subnet[var.profile]), length(data.aws_instances.provisioned_instances.ids) % 3) : var.subnet
}
